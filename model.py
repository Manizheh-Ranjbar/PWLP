...
class Net(torch.nn.Module):
    def __init__(self, input_dim, hidden_size, latent_dim=[32, 32, 32], with_dropout=False, len_nodeattribute=0):
        ...
        self.linear1 = nn.Linear(latent_dim+len_nodeattribute, hidden_size)
    ...
    def forward(self, data):
        ...
        # print(cur_message_layer.dtype)
        # nodefeatures = nodefeatures.to(cur_message_layer.dtype)
        if data.average_feature is not None: 
            nodefeatures=data.average_feature
            # print(nodefeatures.shape)
            cur_message_layer = torch.cat([cur_message_layer, nodefeatures], 1)
        # print(cur_message_layer.shape)
